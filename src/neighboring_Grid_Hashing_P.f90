      ...
      subroutine neighboring_Grid_Hashing_P(rdomain,mspace,ntype,npairs, &
...
!      dzmax = 1.
      ...
      ncelda_y = int((dymax - dymin) / cell_size)
      ncelda_z = int((dzmax - dzmin) / cell_size)

!      write(*,*)'ncelda_x, ncelda_y, ncelda_z',ncelda_x, ncelda_z


      call search_p(rdomain,mspace,ntype,npairs,pairs,nfilas,mrij,mxij, &
          mvij,mvxij,ncelda_x,ncelda_y,ncelda_z,cell_size,ndomain+1,itimestep)


      
    end subroutine neighboring_Grid_Hashing_P

!c     ---------------------------------------------------------------------

      subroutine search_p(rdomain,mspace,ntype,npairs,pairs,nfilas,mrij, &
...
!      double precision dxmin, dxmax, dymin, dymax, dzmin, dzmax
      ...
      integer :: part, start_idx, end_idx, nm
!      integer, parameter :: np = 2 ! ------------------------------ Número de procesadores
      integer :: segment_size, remainder

      if (allocated(celdas)) then
        deallocate(celdas)
      end if
      ! Asignar memoria
      allocate(celdas(ncelda_x, ncelda_y, ncelda_z, nceldas))
      ...
      do i = 1, ntype(1)
         ...
         cell_y = int((mspace(3,i) - dymin) * ymin_inv) + 1
         cell_z = int((mspace(4,i) - dzmin) * zmin_inv) + 1
         if (cell_x > 0 .and. cell_x <= ncelda_x .and. &
              cell_y > 0 .and. cell_y <= ncelda_y .and. &
              cell_z > 0 .and. cell_z <= ncelda_z) then
              ! Añadir la partícula a la celda correspondiente
            sceldas(cell_x, cell_y, cell_z) = sceldas(cell_x, cell_y, cell_z) + 1
            celdas(cell_x, cell_y, cell_z, sceldas(cell_x, cell_y, cell_z)) = &
                 int(mspace(1,i))
            if(sceldas(cell_x, cell_y, cell_z).gt.20)then
               write(*,*)'There are cells with more than 20 particle'
               write(*,*)'Number ',sceldas(cell_x, cell_y, cell_z)
      ...
      if (cell_x < 1 .or. cell_x > ncelda_x .or. &
           cell_y < 1 .or. cell_y > ncelda_y .or. &
           cell_z < 1 .or. cell_z > ncelda_z) then
         write(*,*) 'Índice fuera de rango: ', cell_x, cell_y, cell_z
         stop
      endif
...
!      write(*,*)'00',ntype
      ...
      ! Calcular el tamaño de cada segmento y el sobrante
      nm = ntype(1) - ntype(2)
      segment_size = (nm) / np
      remainder = mod(nm, np)
      
      ! Imprimir cómo se dividirán los índices
!      print *, nm,"Dividiendo en segmentos de tamaño:", segment_size
!      print *, "Sobrante a repartir en el último segmento:", remainder

      
      !$OMP PARALLEL DO PRIVATE(i,j,part,start_idx,end_idx,cell_x,cell_y,cell_z,dx,dy,dz,neighbor_x,neighbor_y,neighbor_z,k,rij,xij,yij,zij,vij,vxij,vyij,vzij)

      do  part = 1, np
         start_idx = (part - 1) * segment_size + 1
         end_idx = part * segment_size
         
         ! Agregar el sobrante al último segmento
         if (part == np) then
            end_idx = nm
         end if

         ! Imprimir el rango del segmento
         !print *, "Parte", part, ":", start_idx, "a", end_idx
         
         ! Procesar el segmento               
      
      do i = start_idx, end_idx!1, ntype(1) - ntype(2)
         ! Calcular la celda de la partícula
         nfilas(i) = 0
         cell_x = int((mspace(2,i) - dxmin) / cell_size) + 1
         cell_y = int((mspace(3,i) - dymin) / cell_size) + 1
         cell_z = int((mspace(4,i) - dzmin) / cell_size) + 1
         ! Buscar en las celdas vecinas
         do dx = -ndomain, ndomain
            do dy = -ndomain, ndomain
               do dz = -ndomain, ndomain
                  ...
                  if (neighbor_x >= 1 .and. neighbor_x <= ncelda_x.and. &
                       neighbor_y >= 1 .and. neighbor_y <= ncelda_y.and. &
                       neighbor_z >= 1 .and. neighbor_z <= ncelda_z)then
                       
                     !     Buscar partículas en la celda vecina
                     do k = 1,sceldas(neighbor_x, neighbor_y,  neighbor_z)
                        if (celdas(neighbor_x, neighbor_y, neighbor_z, k) &
                             .ne. -1) then
                           !     Calcular la distancia entre las partículas i y j
                           j = celdas(neighbor_x, neighbor_y, neighbor_z, k)
                           if(i.ne.j)then
