      ...
      subroutine neighboring_Grid_Hashing_FVC(rdomain,mspace,ntype,npairs, &
...
!      write(*,*)'ncelda_x, ncelda_y, ncelda_z',ncelda_x, ncelda_z
      ...
      call search_fvc(rdomain,mspace,ntype,npairs,pairs,nfilas,mrij,mxij, &
          mvij,mvxij,ncelda_x,ncelda_y,ncelda_z,cell_size,ndomain+1,itimestep)


      
    end subroutine neighboring_Grid_Hashing_FVC

!c     ---------------------------------------------------------------------

      subroutine search_fvc(rdomain,mspace,ntype,npairs,pairs,nfilas,mrij, &
      ...
      integer pairs(npairs,ntype(1)),nfilas(ntype(1)),i,j,d
      double precision mspace(26,nmax)
      double precision rij,xij,yij,zij,rboundmin
      double precision mrij(npairs,ntype(1)),mxij(3,npairs,ntype(1))
      double precision vij,vxij,vyij,vzij,rdomain
      double precision mvij(npairs,ntype(1)),mvxij(3,npairs,ntype(1))
...
!      write(*,*)'gg'
      ...
      if (cell_x < 1 .or. cell_x > ncelda_x .or. &
         ...
         write(*,*) 'Índice fuera de rango: ', cell_x, cell_y, cell_z,ncelda_x,ncelda_y,ncelda_z
         stop
      endif
...
!      write(*,*)'hh'
      ...
      do i = 1, ntype(1) - ntype(2)
         ...
         do dx = -ndomain, ndomain
                     ...
                     do k = 1,sceldas(neighbor_x, neighbor_y,  neighbor_z)
                        if (celdas(neighbor_x, neighbor_y, neighbor_z, k) &
                           ...
                           if(i.ne.j)then
                              ...
                              if(mspace(12,j).eq.-1.and.rij.lt.rboundmin)then
                                 rboundmin = rij
                              endif
            ...
            enddo
         enddo
         !if(rboundmin.lt.0.3)write(*,*)i,rboundmin
         mspace(17,i) = rboundmin
      enddo

    deallocate(celdas)  ! Libera memoria al final

    end subroutine search_fvc
