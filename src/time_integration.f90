...
!c     mspace(17,i) = Distancias menores entre particula fluido y particula frontera     [out]
!c     mspace(18,i) = Parametro K (viscosidad) dependiente de la concentracion  [out]
!c     mspace(19,i) = Parametro exponente n de Herschel-Buljey
...
!      angle = 90.
      ...
      if(source_mode == 'y')then
         ...
         T_emision = 1.5 * h0 /abs(v_fuente) 
         volumen_emitido = QQ * T_emision
         masa_total = rho0 * volumen_emitido
         N_horizontal = int(d_horizontal / h0)
         d_vertical = QQ / (abs(v_fuente) * d_horizontal)
         N_vertical = int(d_vertical / h0)
         if(N_vertical.lt.1)N_vertical = 1      
      ...
      if(source_mode == 'y')then
         ...
         write(*,*) 'Masa emitida = ', masa_total, rho0 * QQ
         write(*,*) 'Volumen emitido = ', Volumen_emitido
         write(*,*) 'Volumen particula = ',Volumen_particula
         write(*,*) 'Masa particula = ',masa_particula,'Densidad Particul =',densidad_particula
...
10    if (mod(itimestep, nstepsf) == 0.and.source_mode == 'y') then
         ...
         do k = 1,N_horizontal
            ...
            call fuente(v_fuente, masa_particula, N_vertical, ntype, mspace, posy)
