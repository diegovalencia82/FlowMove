...
subroutine fuente(vel, masa_particula, num_points, ntype, mspace, posy)
  implicit none
  include 'param.inc'
...
!  double precision, INTENT(IN) :: start_x, start_z, angle
  ...
  ! Generar puntos
  DO i = 1, num_points
     x_coords(i) = start_x + (i - 1) * 1.5*h0 * COS(angle_rad)
     z_coords(i) = start_z + (i - 1) * 1.5*h0 * SIN(angle_rad)
     y_coords(i) = posy
  END DO
...
!     mspace(25,i) =
     mspace(26,i) = mu_new
     write(*,*)int(mspace(1,i)),real(mspace(2,i)),real(mspace(3,i)),real(mspace(4,i)) &    
