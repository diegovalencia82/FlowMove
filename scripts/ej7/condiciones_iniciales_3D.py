...
INCLUIR_FLUIDO = True   # Pon False para que solo aparezcan las partículas frontera
...
def generar_sph_snapshot(X_topo, Y_topo, Z_topo, X_fluido=[], Y_fluido=[], Z_fluido=[], viscFluid=None, n_parameter=None, viscWall=0.001002, deltaf=1, ddx=1, ht=1, x0=0, y0=0, filename='snapshot_000'):
    # Constantes físicas
    g = 9.82
    gamma = 7.0
    beta0 = 1.0
    ...
    # Manejar viscosidad si no se provee
    if viscFluid is None:
        viscF = np.full(num_fluid_particles, 0.001002)  # Valor por defecto
        n_parameter = np.full(num_fluid_particles, 1)  # Valor por defecto
    else:
        viscF = viscFluid
    ...
    with open(filename, 'w') as f:
        ...
        for i in range(num_fluid_particles):
            f.write(f"{ids_fluid[i]}   {X_fluido[i]:.6f}   {Y_fluido[i]:.6f}   {Z_fluido[i]:.6f}   "
                    ...
                    f"{pressure[i]:.6f}   {internal_energy[i]:.6f}   {itype[i]}   {hsml[i]:.6f} {viscF[i]:.6f}   0.0   {n_parameter[i]:.6f}\n")
...
def graficar_lamina_puntos1(puntos, color='b', escala=True,inigraf=True,fingraf=True):
    ...
    # Graficar los puntos
    ax_global.scatter(x, y, z, c=color, s=10, alpha=0.08)
...
# Dimensiones canal
deltaW = 0.015/2  # Espacio entre puntos de pared
largo = 1.20
ancho = 0.5
alto = 0.5

# Dimesiones bloque de fluido
deltaF = 0.015
largof = 0.5
anchof = 0.5
altof = 0.3
...
if INCLUIR_FLUIDO:
    espacio_entre_puntos = deltaF

    # Parámetros del bloque 1 de fluido
    punto_inicial = np.array([espacio_entre_puntos*1.5, espacio_entre_puntos*1.5, espacio_entre_puntos*1.5])
    bloque1 = generar_bloque_puntos(
        punto_inicial = punto_inicial,
        lado1 = largof-espacio_entre_puntos,  lado2 = anchof-espacio_entre_puntos*3, altura = altof-espacio_entre_puntos*3,
        angulo_azimutal = 0, angulo_zenital = 0,
        espacio_entre_puntos = espacio_entre_puntos,
        plano = "x-y"
    )
    print('Number fluid bloque 1 = ',len(bloque1))

    # Segundo bloque de fluido (viscosidad alta)
    punto_inicial2 = np.array([0.8, 0.2, espacio_entre_puntos*1.5])
    bloque2 = generar_bloque_puntos(
        punto_inicial=punto_inicial2,
        lado1=0.2, lado2=0.2, altura=altof-espacio_entre_puntos*3,
        angulo_azimutal=0, angulo_zenital=0,
        espacio_entre_puntos=espacio_entre_puntos,
        plano="x-y"
    )
    print('Number fluid bloque 2 = ',len(bloque2))

    # Combinar ambos bloques
    bloque_total = np.vstack([bloque1, bloque2])
    X_fluido = bloque_total[:,0]
    Y_fluido = bloque_total[:,1]
    Z_fluido = bloque_total[:,2]

    # Crear array de viscosidades (0.001 para el primer bloque, 0.01 para el segundo)
    visc_fluidos = np.concatenate([
        np.full(len(bloque1), 0.001), 
        np.full(len(bloque2), 0.5)
    ])

    # Crear array de n parameter (1 para el primer bloque, 1.5 para el segundo)
    n_parameter = np.concatenate([
        np.full(len(bloque1), 1), 
        np.full(len(bloque2), 1.5)
    ])

    ht = np.max(Z_fluido)
        
else:
    X_fluido, Y_fluido, Z_fluido = [], [], []
    visc_fluidos = []
    ht = 0.0
...
lado1 = largo  # Longitud del primer lado de la lámina, logitud real 7.1
lado2 = ancho   # Longitud del segundo lado de la lámina
angulo_azimutal = 0  # Ángulo azimutal en grados
angulo_zenital =  0  # Ángulo zenital en grados
espacio_entre_puntos = deltaW # Espacio entre puntos en la lámina
...
punto_inicial = np.array([0, 0, deltaW])
lado1 = alto  # Longitud del primer lado de la lámina
lado2 = largo   # Longitud del segundo lado de la lámina
angulo_azimutal = 0  # Ángulo azimutal en grados
angulo_zenital =  0  # Ángulo zenital en grados
espacio_entre_puntos = deltaW # Espacio entre puntos en la lámina
...
punto_inicial = np.array([0, ancho, deltaW])
lado1 = alto  # Longitud del primer lado de la lámina
lado2 = largo   # Longitud del segundo lado de la lámina
angulo_azimutal = 0  # Ángulo azimutal en grados
angulo_zenital =  0  # Ángulo zenital en grados
...
punto_inicial = np.array([0, deltaW, 0+deltaW])
lado1 = alto  # Longitud del primer lado de la lámina
lado2 = ancho-deltaW*2   # Longitud del segundo lado de la lámina
angulo_azimutal = 0  # Ángulo azimutal en grados
angulo_zenital =  0  # Ángulo zenital en grados
espacio_entre_puntos = deltaW # Espacio entre puntos en la lámina
...
punto_inicial = np.array([largo-deltaW/2, deltaW, deltaW])
lado1 = alto  # Longitud del primer lado de la lámina
lado2 = ancho-deltaW*2   # Longitud del segundo lado de la lámina
angulo_azimutal = 0  # Ángulo azimutal en grados
angulo_zenital =  0  # Ángulo zenital en grados
espacio_entre_puntos = deltaW # Espacio entre puntos en la lámina
lamina14 = generar_lamina_puntos(punto_inicial, lado1, lado2, angulo_azimutal, angulo_zenital, espacio_entre_puntos, plano="z-y")
...
if INCLUIR_FLUIDO:
    graficar_lamina_puntos1(bloque_total, color='brown',escala=True,inigraf=True,fingraf=True)

graficar_lamina_puntos1(puntos, color='gray',escala=True,inigraf=True,fingraf=False)
if INCLUIR_FLUIDO:
    graficar_lamina_puntos1(bloque_total, color='brown',escala=False,inigraf=False,fingraf=True)

#puntos = transformar_puntos(puntos, theta=45, phi=0, traslacion=[0, 0, 1.8])
#bloque_puntos = transformar_puntos(bloque_puntos, theta=45, phi=0, traslacion=[0, 0, 1.8])
...
if INCLUIR_FLUIDO:
    ht = np.max(Z_fluido)#-np.min(Z_fluido)
...
generar_sph_snapshot(x, y, z, X_fluido, Y_fluido, Z_fluido, viscFluid=visc_fluidos, viscWall=0.0, n_parameter=n_parameter, deltaf=deltaF, ddx=deltaW, ht=ht, x0=0, y0=0, filename='snapshot_000')
