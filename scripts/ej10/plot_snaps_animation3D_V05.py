...
def update(frame):
    ...
    # Solo cuando sel==1 usamos los puntos de la primera topografía
    if sel == 1 and datos_topografia is not None:
        mountain_particles = datos_topografia[::20]
    else:
        mountain_particles = None
    ...
    if sel == 1:
        ...
        Xh_grid, Yh_grid, Zh_grid = interpolar_puntos(mountain_particles[:, 1:4], num_puntos=100, metodo='cubic')
        ax_3d.plot_surface(Xh_grid, Yh_grid, Zh_grid, color='green', alpha=0.7, edgecolor='none')
    
    ax_3d.text2D(0.05, 0.95, f't = {frame[0]:.2f}', transform=ax_3d.transAxes, ha='left', va='top', fontsize=12)


def main():
    global sel, tamanop, ht, x_min, x_max, y_min, y_max, z_min, z_max, ax_3d, ax_xz, datos_topografia
    ...
    ht = float(input("Altura total (ht) (presiona Enter para usar 0.5): ") or "5")
    x_min = float(input("Límite mínimo en X (presiona Enter para usar 167): ") or "167")
    x_max = float(input("Límite máximo en X (presiona Enter para usar 1151): ") or "1151")
    y_min = float(input("Límite mínimo en Y (presiona Enter para usar 914): ") or "914")
    y_max = float(input("Límite máximo en Y (presiona Enter para usar 2338): ") or "2338")
    z_min = float(input("Límite mínimo en Z (presiona Enter para usar 1577): ") or "1577")
    z_max = float(input("Límite máximo en Z (presiona Enter para usar 2121): ") or "2121")

    folder = input("Carpeta de los archivos snapshot (Presiona Enter para usar ./): " or "./")
    num_snapshots = int(input("Número de snapshots a leer (presiona Enter para usar 50): ") or "50")
