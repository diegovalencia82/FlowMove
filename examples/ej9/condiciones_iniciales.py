    ...
    with open(filename, 'w') as f:
        ...
        for i in range(num_fluid_particles):
            f.write(f"{ids_fluid[i]}   {X_fluido[i]:.6f}   {Y_fluido[i]:.6f}   {Z_fluido[i]:.6f}   "
                    ...
                    f"{pressure[i]:.6f}   {internal_energy[i]:.6f}   {itype[i]}   {hsml[i]:.6f} {viscF[i]:.6f}   4.02   1.5\n")
        ...
        for i in range(num_wall_particles):
            ...
            f.write(f"{ids_wall[i]}   {X_topo[i]:.6f}   {Y_topo[i]:.6f}   {Z_topo[i]:.6f}   "
                    ...
                    f"{pressure[j]:.6f}   {internal_energy[j]:.6f}   {itype[j]}   {hsml[j]:.6f} {viscW[i]:.6f}   4.02   1.5\n")
...
def graficar_lamina_puntos1(puntos, color='b', escala=True,inigraf=True,fingraf=True):
    ...
    # Graficar los puntos
    ax_global.scatter(x, y, z, c=color, s=10, alpha=0.05)
...
generar_sph_snapshot(x, y, z, X_fluido, Y_fluido, Z_fluido, viscFluid=4.02, viscWall=4.02, deltaf=delta*2, ddx=delta, ht=ht, x0=0, y0=0, filename='snapshot_000')
