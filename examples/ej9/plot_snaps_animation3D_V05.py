...
def crear_caja(x_min, x_max, y_min, y_max, z_min, z_max):
    """
    Crea los vértices y caras de una caja 3D.
    
    Retorna:
    - vertices: array 8x3 de coordenadas.
    - caras: lista de caras (índices de vértices).
    """
    vertices = np.array([
...
def dibujar_caja(ax, vertices_transformados, caras):
    """
    Dibuja una caja 3D usando vértices transformados.
    
    Parámetros:
    - ax: eje 3D.
    - vertices_transformados: array 8x3 de vértices transformados.
    - caras: lista de caras (índices de vértices).
    """
    # Crear caras con coordenadas transformadas
    caras_coords = []
    for cara in caras:
        cara_coords = [vertices_transformados[i] for i in cara]
        caras_coords.append(cara_coords)
...
def update(frame):
    ...
    # Generar vértices y caras originales de la caja
    vertices_originales, caras = crear_caja(x_min, x_max, y_min, y_max, z_min, z_max)
    
    # Transformar TODOS los vértices
    vertices_transformados = transformar_puntos(
        puntos=vertices_originales,
        theta=45,     # Rotación zenital (ejemplo)
        phi=0,        # Rotación azimutal (ejemplo)
        traslacion=[0, 0, 1.8]  # Traslación (ejemplo)
    )
    
    # Dibujar la caja transformada
    dibujar_caja(ax_3d, vertices_transformados, caras)
...
def main():
    ...
    x_max = float(input("Límite máximo en X (presiona Enter para usar 2.5): ") or "2.5")
    y_min = float(input("Límite mínimo en Y (presiona Enter para usar 0.0): ") or "0.0")
    y_max = float(input("Límite máximo en Y (presiona Enter para usar 0.3): ") or "0.3")
