...
def plot_snapshots_2d(data_frames, num_snapshots, rows, cols):
    ...
    for i in range(rows):
        for j in range(cols):
            ...
            cmap = plt.get_cmap('copper')#coolwarm
            sm = ScalarMappable(norm=norm, cmap=cmap)
            colors = sm.to_rgba(velocity)
            
            # Graficar las partículas
            ax.scatter(fluid_particles[:, 1], fluid_particles[:, 3], color=colors, s=tamanop, alpha=0.8)
    ...
    plt.savefig('fig_snapshots_2d_lodo_HB.png', bbox_inches='tight')
    plt.show()
