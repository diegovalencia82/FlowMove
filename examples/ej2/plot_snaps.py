...
def transformar_puntos(puntos, theta=0, phi=0, traslacion=[0, 0, 0]):
...
def plot_snapshot(ax_3d, ax_xz, frame, sel, tamanop, ht, x_min, x_max, y_min, y_max, z_min, z_max):
    ...
    #colors = plt.cm.coolwarm(norm(velocity))  
    colors = plt.cm.winter(norm(velocity))
...
def main():
    ...
    sm = ScalarMappable(norm=Normalize(0, np.sqrt(2*9.8*ht)), cmap='winter') # coolwarm
    plt.colorbar(sm, cax=cbar_ax, orientation='horizontal', label='Magnitud de Velocidad')
    
    plt.savefig('fig_snapshots_agua3D.png', dpi=300, bbox_inches='tight')
    print("Figura guardada como 'fig_snapshots_agua3D.png'")
    plt.close()
